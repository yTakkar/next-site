...
On platforms like [ZEIT Now](https://zeit.co), `gzip` and `brotli` are handled automatically on the proxy level.
...
If you're deploying on a platform that handles compression such as [ZEIT Now](https://zeit.co/home) no changes are needed.
...
ZEIT has a few large Next.js applications. For example, [zeit.co](https://zeit.co/), [zeit.co/docs](http://zeit.co/docs), and [nextjs.org](http://nextjs.org/). We've been [dogfooding](https://en.wikipedia.org/wiki/Eating_your_own_dog_food) Next.js internally at ZEIT and improved Next.js based on our experiences.
...
You can read more about telemetry and how to opt-out if you don't want to participate [here](https://nextjs.org/telemetry).
...
The Next.js community on spectrum.chat/next-js has doubled since the last major release with over 10,900 members. Join us!
