...
[Alex Castle](https://twitter.com/atcastle) [proposed a new method of chunking](https://github.com/zeit/next.js/issues/7631) (creation of separate JavaScript files) that allows more optimized commons chunking with multiple files, especially when many pages are involved.

Today, we're happy to announce this new chunking behavior is on-by-default in Next.js 9.2.
...
The new chunking implementation leverages [HTTP/2](https://developers.google.com/web/fundamentals/performance/http2) to deliver more, smaller chunks.
...
Furthermore, [Barnebys®](https://www.barnebys.com/) saw a 87% reduction in the amount of JavaScript loaded after six (6) page navigations:

<h4>JavaScript Loaded by Client-Side Transitions</h4>
<table>
  <thead>
    <tr>
      <th>&nbsp;</th>
...
`slug` will be provided in the router query object as an array of individual path parts, for example: `/post/example-category/hello-world`.
The Router's `query` object will hold `{ slug: ['example-category', 'hello-world'] }`.
