...
Preview Mode is already available when using `next start`, or seamlessly by deploying to the [ZEIT Now](https://nextjs.org/docs/deployment) CDN.
...
CSS Modules for Sass files are an *optional* feature and are only enabled for files with the `.module.scss` extension. Regular `[<link>` stylesheets](https://nextjs.org/docs/api-reference/next/head) and [g](https://nextjs.org/blog/next-9-2#built-in-css-support-for-global-stylesheets)lobal Sass styles are still supported.
