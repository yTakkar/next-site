...
For more details, check out
[the `basePath` documentation](https://nextjs.org/docs/api-reference/next.config.js).
...
module.exports = {
  async rewrites() {
    return [
      ...
      {
        source: '/:path*',
        destination: `https://legacy.com/:path*`
      }
    ]
  }
}
...
[example.com](http://example.com) which would be the previous project.
