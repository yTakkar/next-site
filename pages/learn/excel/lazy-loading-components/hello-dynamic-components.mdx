...
Now we are trying to convert the above `Highlight` component into a **dynamic component**. Dynamic components are powered by the [dynamic imports](https://nextjs.org/docs/advanced-features/dynamic-import) in Next.js.

Most importantly, these components will be loaded only when they are about to be rendered in your app.

Creating a dynamic component is simple. You can use the `next/dynamic` utility.

Here's how you can convert `Highlight` into a dynamic component:
