...
export default () => {
  const isAmp = useAmp()
  const Img = props => React.createElement(
    isAmp ? 'amp-img' : 'img',
    {
      ...props,
      ...(isAmp ? {
        ...
        <div className="slides">
          {DATA.slice(0, DATA.length - 1).map((item, i) => {
            return <div className="slide" key={`thumbnail-${i}`}>
              <Img src={item.src.replace('/showcases/', '/showcase-thumbnails/')} alt={`Showcase ${i}`} height={imgHeight} width={imgWidth} layout='responsive' />
            </div>
          })}
        </div>
      </div>
