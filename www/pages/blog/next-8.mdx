...
<Image caption={<>The Next.js serverless target outputs Serverless functions from pages</>} captionSpacing={30} video src="/static/blog/next-8/static-exporting.mp4" width={2102 / 2} height={688 / 2} />
...
<ReactTweet id="1085398982753468416" />
...
With Next.js 8 we are introducing a new key to `next.config.js` named `env` to provide the same functionality in a backwards compatible way:
...
`process.env.customEnv` will be replaced with `'MyValue'` at build time.
...
Whenever a error occurs in production the `/_error` page is rendered to display that an error occurred.
...
<Image caption={<>The on-demand entries flow</>} captionSpacing={30} src="/static/blog/next-8/flow.jpg" width={966 / 2} height={1061 / 2} />
...
Ever since it's first release Next.js has been used in everything from fortune 500 companies to personal blogs. We're very excited to see the continued growth in Next.js adoption.
...
The Next.js community on [spectrum.chat/next-js](https://spectrum.chat/next-js) has over **4,570 members**. [Join us!](https://spectrum.chat/next-js)
